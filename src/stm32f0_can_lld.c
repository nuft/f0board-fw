 ...
 * @isr
 */
OSAL_IRQ_HANDLER(STM32_CAN1_HANDLER) {

  OSAL_IRQ_PROLOGUE();

  if (CAND1.can->TSR & (CAN_TSR_RQCP0 | CAN_TSR_RQCP1 | CAN_TSR_RQCP2)) {
      can_lld_tx_handler(&CAND1);
  }
  if (CAND1.can->RF0R & (CAN_RF0R_FMP0 | CAN_RF0R_FOVR0)) {
    can_lld_rx0_handler(&CAND1);
  }
  if (CAND1.can->RF1R & (CAN_RF1R_FMP1 | CAN_RF1R_FOVR1)) {
    can_lld_rx1_handler(&CAND1);
  }
  if (CAND1.can->MSR & (CAN_MSR_WKUI | CAN_MSR_ERRI | CAN_MSR_SLAKI)) {
...
#if STM32_CAN_USE_CAN1
  if (&CAND1 == canp) {
    nvicEnableVector(STM32_CAN1_NUMBER, STM32_CAN_CAN1_IRQ_PRIORITY);
    /*
...
void can_lld_stop(CANDriver *canp) {
  ...
  if (canp->state == CAN_READY) {
    ...
    if (&CAND1 == canp) {
      ...
      nvicDisableVector(STM32_CAN1_NUMBER);
      /*
